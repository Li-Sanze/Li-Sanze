...
> **修订：2026-04-28 状态同步**
> - CrossReview `0.1.0a1` 已发布并完成 post-release smoke（PyPI clean venv install + host-integrated CLI）
> - Sopify Phase 4a advisory skill 草拟已完成（SKILL.md + skill.yaml + develop-rules `post_develop` 触发）
> - 当前下一步收敛为 Sopify Phase 4a E2E + 3 项目 dogfood；Phase 4b 继续冻结
...
─────────────────    ─────────────────    ─────────────────   ─────────────────   ─────────────────
Sopify Phase 4a        Protocol Step 1      BlueprintEnhancer   knowledge-graph     CR Phase 4b
E2E + 3 项目 dogfood   (ADR-016 基础)       ABC 实现             -query Skill         (不进入 0-6 月)
CR debug JSONL         Protocol Step 3      GraphifyEnhancer    规则沉淀管线
trace（0.1.0a1 后）     (ADR-017 约束)       MVP                 反馈闭环设计
...
#### T-P0-3：CR v0-05 / v0-06 / v0-07 发布就绪链 `✅ 完成`
> 从 T-P1-1 拆出并提升为 P0（CR 发布关键路径，与 CR 总纲一致）

- **背景：** release gate 通过后，仍需补齐用户可运行的发布就绪能力；2026-04-28 已随 `crossreview==0.1.0a1` 发布并完成 smoke。
- **行动：**
  1. v0-05：human-readable 输出 (`--format human`)
  2. v0-06：one-stop verify (`crossreview verify --diff`)
  3. v0-07：发布就绪判定
- **产出：** CrossReview v0 release ready；PyPI alpha `0.1.0a1` 可安装。
- **依赖：** T-P0-2
- **估计：** 已完成

#### T-P0-4：CR debug JSONL trace `P1 / 0.1.0a1 后第一个维护迭代`
...
#### T-P1-1：CrossReview v0 alpha 发布 `✅ 完成`
> 原 T-P2-4，提升为 P1（P0 gate 通过后的直接下一步）

- **背景：** v0-07 发布就绪后的分发收尾工作；2026-04-28 已完成 `0.1.0a1` 发布和安装 smoke。不是 stable `0.1.0`。
- **行动：**
  1. PyPI `0.1.0a1` 发布
  2. 文档 + 示例
  3. 安装路径验证
- **产出：** `pip install crossreview==0.1.0a1` 可用；host-integrated CLI (`pack` / `render-prompt` / `ingest --format human`) smoke 通过。
- **依赖：** T-P0-3
- **估计：** 已完成

#### T-P1-2：Sopify Phase 4a — CR Advisory 集成 + Convention 模式验证 `当前下一步`
...
- **当前状态：** SKILL.md + skill.yaml + develop-rules `post_develop` 触发已完成；剩余工作是 E2E 跑通和 3 项目 dogfood 数据采集。
- **战略双重定位：**
...
- **产出：** Sopify develop 完成后可触发 CrossReview 审查；待 E2E 和 3 项目 dogfood 证明稳定性。
- **依赖：** T-P1-1
- **估计：** dogfood 1-3 天；后续是否升级 Validator 由数据决定
...
T-P0-1 ✅ (CR unclear_rate)
    └──→ T-P0-2 ✅ (release gate 全量重跑)
            └──→ T-P0-3 ✅ (v0-05/06/07 发布就绪)
                    ├──→ T-P0-4 (debug JSONL trace, 默认关闭, 0.1.0a1 后)
                    └──→ T-P1-1 ✅ (CR PyPI 0.1.0a1)
                            └──→ T-P1-2 当前 (Sopify Phase 4a E2E + 3 项目 dogfood)
...
- [x] v0-05 / v0-06 / v0-07 发布就绪链通过

### M2：CR 正式发布 + Phase 4a + Protocol 基础（+4 周）
- [x] CR v0 alpha 发布（PyPI `0.1.0a1`）+ post-release smoke 通过
...
| D-A11 | Verify Loop 不独立建项目 | verify-loop 核心抽象（artifact_pack / rubric / evaluator / verdict / run_result / stop_policy）与 CrossReview 的 ReviewPack / ReviewResult 高度重合（6/7 概念对应）。好想法吸收进 CrossReview：stop_policy → loop mode (v0.5+)，rubric → focus 增强 (v1+)，multi-artifact → ArtifactType 扩展 (v1+)。evidentloop org 下不单独建 verify-loop 仓库 |
